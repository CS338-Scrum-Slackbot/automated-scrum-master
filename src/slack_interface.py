    ...
    # A view submission payload is received when a user submits a modal
        ...
        try:
            ...
            if callback_id == "start-sprint-modal":
                ...
                schedule_sprint_end_message(list(data['view']['state']['values'].values()))
...
def end_sprint():
    ...
    sprint_duration = scrum_master.scrum_board.read_metadata_field('sprint_duration')
...
def extend_sprint():
    ...
    success = schedule_message(new_end, blocks=end_msg['blocks'])
    if success:
        scrum_master.save_sched_message('end_sprint', success["scheduled_message_id"])
...
def schedule_sprint_end_message(payload_values):
    ...
    scrum_master.handle_sprint_submission(unix_start, unix_end, success=success)
