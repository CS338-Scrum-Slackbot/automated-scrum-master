...
class ScrumBoard:
    ...
    def delete_story(self, story_id):
        ...
        if moved_story == None or new_log == None:
            ...
            return "Successfully deleted " + "\"ID " + str(story_id) + ": " + moved_story["story"] + "\"" + " from " + f'`{source_log}`'
    ...
    def delete_swimlane(self, log_name: str):
        ...
        if result == -3:
            ...
            return f"`{log_name}` does not exist."
        elif result == -1:
            return f"`{log_name}` is a default log and may not be deleted."
        elif result == 0:
            return f"Failed to move stories from `{log_name}` before deletion."
  ...
  # ==========
    ...
    def write_metadata_field(self, field: str, value: any):
        if self.reader.write_metadata_field(field, value):
            ...
            raise RuntimeError(
                f"Failed to write {value} to {field} in json metadata")
